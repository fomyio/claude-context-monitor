...
if [ -f "\$PLUGIN_SCRIPT" ]; then
  ...
  # Plugin removed — remove statusLine from settings.json and self-destruct
  node -e 'const fs=require("fs"),p=require("path"),f=p.join(process.env.HOME,".claude/settings.json");try{const s=JSON.parse(fs.readFileSync(f,"utf8"));delete s.statusLine;fs.writeFileSync(f,JSON.stringify(s,null,2));}catch(_){}' 2>/dev/null
  rm -f "\$0"
fi
EOF
